...
class SSG:
    ...
    def function(self, central_wav, width, s, amplitude):
        if (central_wav - width > np.min(self.wavelengths)) and (
            central_wav + width < np.max(self.wavelengths)
        ):
            DN = amplitude * np.exp(
                -np.abs(self.wavelengths - central_wav) ** s / (2 * width)
            )
            return DN
        else:
...
class TestMCMCRetrieval(unittest.TestCase):
    ...
    def test_single_inputqty_0D(self):
        retr = MCMCRetrieval(
            function,
            y[0],
            rand_uncertainty=y_err[0],
            parallel_cores=20,
            initial_guess=x[0] + 1,
            downlims=0.0,
        )
        medians, unc, samples = retr.run_retrieval(
            150, 300, 20, return_samples=True, return_corr=False
        )  # ,x_0=x[0])
    ...
    def test_single_inputqty_1D(self):
        retr = MCMCRetrieval(
            function,
            y,
            rand_uncertainty=y_err,
            parallel_cores=20,
            initial_guess=x,
            n_input=1,
        )
        medians, unc, samples = retr.run_retrieval(
            200, 800, 200, return_samples=True, return_corr=False
        )  # ,x_0=x)
        print(medians, unc)
    ...
    def test_multiple_inputqty_1D(self):
        retr = MCMCRetrieval(
            function_b,
            y_b[0],
            rand_uncertainty=yerr_b_uncorr[0],
            parallel_cores=20,
            initial_guess=[x1_b[0], x2_b[0]],
            n_input=2,
        )
        medians, unc, samples = retr.run_retrieval(
            200, 800, 200, return_samples=True, return_corr=False
    ...
    def test_class_function(self):
        ...
        retr = MCMCRetrieval(
            ssg.function,
            DN,
            rand_uncertainty=DN_unc,
            parallel_cores=3,
            initial_guess=[725, 3, 2, 35000],
            downlims=[700.0, 2.0, 1.5, np.max(DN) * 0.9],
            uplims=[750, 10, 4, np.max(DN) * 1.1],
            n_input=4,
        )
